...
import {
    articleCommentsBlockActions,
    articleCommentsBlockReducer,
} from '../model/slice/articleCommentsBlockSlice';
...
const ArticleCommentsBlock = memo((props: ArticleCommentsBlockProps) => {
    ...
    const onSubmitCommentFormHandler = useCallback(
        (content: string) => {
            dispatch(addCommentToArticle({ articleId, content })).then(() =>
                dispatch(addCommentFormActions.setContent('')),
            );
        },
        [articleId, dispatch],
    );
    ...
    return (
        <DynamicModuleLoader reducers={initialReducers}>
            <div
                className={classNames(cls.articleCommentsBlockWrapper, {}, [
                    className,
                ])}
            >
                <div className={cls.commentListHeader}>
                    ...
                    <Tag>
                        {totalComments} {t('comment_block_total')}
                    </Tag>
                </div>
                <AddCommentForm
                    loading={formLoading}
