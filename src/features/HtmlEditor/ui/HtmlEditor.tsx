...
export const HtmlEditor = memo((props: HtmlEditorProps) => {
  const {
    html = '', onChangeContent, className, showPreview = false,
  } = props;
  ...
  const handleImageUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && editorRef.current) {
      ...
      reader.onload = (e) => {
        ...
        if (selection && selection.rangeCount > 0) {
          const range = selection.getRangeAt(0);
          range.insertNode(document.createRange().createContextualFragment(imgElement));
          if (editorRef.current) {
            onChangeContent?.(editorRef.current.innerHTML);
            setTimeout(() => restoreSelection(savedRange), 0);
          }
  ...
  const handleList = useCallback((type: 'ul' | 'ol') => {
    ...
    if (selection && selection.rangeCount > 0 && editorRef.current) {
      const range = selection.getRangeAt(0);
      const selectedText = range.toString();
      const items = selectedText.split('\n').filter((item) => item.trim()).map((item) => `<li>${item}</li>`);
      const listElement = `<${type}>${items.join('')}</${type}>`;
      range.deleteContents();
      range.insertNode(document.createRange().createContextualFragment(listElement));
      onChangeContent?.(editorRef.current.innerHTML);
      setTimeout(() => restoreSelection(savedRange), 0);
    }
  }, [onChangeContent, restoreSelection, saveSelection]);
  ...
  return (
    <Card className={classNames(styles.editorContainer, {}, [className])}>
      <div className={styles.toolbar}>
        ...
        <Tooltip content="Маркированный список" preferredPlacement="top">
          <Button
            theme="ghostIcon"
        ...
        <Tooltip content="Загрузить изображение" preferredPlacement="top">
          <Button
            theme="ghostIcon"
