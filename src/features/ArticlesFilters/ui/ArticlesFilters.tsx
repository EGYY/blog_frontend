...
import { getArticleFilterSelectedCategories } from '../model/selectors/getArticleFilterSelectedCategories/getArticleFilterSelectedCategories';
import { getArticleFilterSelectedTags } from '../model/selectors/getArticleFilterSelectedTags/getArticleFilterSelectedTags';
...
export const ArticlesFilters = memo((props: ArticlesFiltersProps) => {
    ...
    const debouncedSearchFunc = useCallback(
        (val: string) => {
            if (val) {
    ...
    const onChageCategories = useCallback(
        (val: string[]) => {
            dispatch(articlesFiltersActions.setCategories(val));
            if (val.length > 0) {
                dispatch(
                    articlesFiltersActions.setFilter({
                        categoryIds: val.join(','),
                    }),
                );
            } else {
    ...
    const onChageTags = useCallback(
        (val: string[]) => {
            ...
            if (val.length > 0) {
                dispatch(
                    articlesFiltersActions.setFilter({ tagIds: val.join(',') }),
                );
            } else {
    ...
    const onChageBase = useCallback(
        (val: 'asc_date' | 'desc_date' | 'most_popular') => {
            let sortBy = 'date';
            let sort = 'desc';
            switch (val) {
                case 'asc_date':
        ...
        >
            ...
            <div className={cls.articleFiltersSelectWrapper}>
                ...
                <Select
                    ...
                    onChange={(val) =>
                        onChageBase(
                            val as 'asc_date' | 'desc_date' | 'most_popular',
                        )
                    }
                    options={filterBase}
