...
const RegistrationForm: FC<RegistrationFormProps> = memo(({ openModal = false, onCloseModal }) => {
  ...
  const handleSubmitForm = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await dispatch(registrateByEmail({ email, password, name }));
  ...
  return (
    <form className={cls.loginForm} onSubmit={handleSubmitForm}>
      <div className={cls.loginFormHeader}>
        <div className={cls.loginFormHeaderTitle}>{t('title_reg')}</div>
        <div className={cls.loginFormHeaderDescription}>{t('description_reg')}</div>
      </div>
      <div className={cls.loginFormContent}>
        ...
        />
        <span className={cls.formAction} onClick={handleChangeFormType}>{t('already_have_account')}</span>
        {error && <p style={{ color: '#e7000b' }}>{error}</p>}
        <Button loading={loading} type="submit">{t('reg_btn')}</Button>
      </div>
    </form>
  );
});
