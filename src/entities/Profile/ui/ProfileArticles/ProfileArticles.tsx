...
export const ProfileArticles = memo((props: ProfileArticlesProps) => {
  const {
    className, articles = [], title, type = 'default', loading = false, onDeleteArticle,
  } = props;
  ...
  const renderArticles = useMemo(() => {
      ...
      return (
        <div className={cls.lastArticleList}>
          {Array.from({ length: 5 }).map((_, index) => (
            <Skeleton width="100%" height={65} key={`article-loading-${index}`} />
          ))}
        </div>
      );
    }
    if (type === 'default') {
      return (
        <div className={cls.lastArticleList}>
          {articles?.length > 0
            ? articles.map((article) => (<ProfileArticleItem key={article.id} article={article} />))
