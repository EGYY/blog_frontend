...
import { ProfileMainInfo, useSubscribe } from '@/entities/Profile';
...
export const Article: FC<ArticleProps> = memo(
    ({ className, article, loadingArticle, errorArticle }) => {
        ...
        const {
            ...
            changeSubscribe: (val) =>
                dispatch(articleActions.changeSubscribe(val)),
        });
        ...
        return (
            <article
                className={classNames(cls.articleWrapper, {}, [
                    className,
                    'printable-article',
                ])}
            >
                <div className={cls.articleHeader}>
                    <HoverCard
                        trigger={
                            <Link
                                to={getRouteProfileDetail(article?.author.id)}
                                className={cls.articleAuthor}
                            >
                                <Avatar
                                    src={`${__SERVER_URL__}${article?.author.avatar}`}
                                />
                                <span>
                                    {article?.author.name ||
                    ...
                    />
                    <div
                        className={classNames(cls.articleHeaderActions, {}, [
                            'printable-actions',
                        ])}
                    >
