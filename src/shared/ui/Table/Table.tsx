...
function TableComponent<T extends Record<string, any>>({
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    setColumns((prevCols) => {
      return prevCols.map((col, idx) => (idx === resizingColIndex.current ? { ...col, width: newWidth } : col));
    });
  }, []);
  ...
  const getLeftOffset = useCallback((index: number) => {
    return columns.slice(0, index).reduce((sum, col) => sum + (col.width || 150), 0);
  }, [columns]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      return visibleColumns.every((col) => {
        ...
        const filterValue = filters[String(col.key)]?.toLowerCase() ?? '';
        const rawCell = row[col.key];
        const cellValue = extractTextFromReactNode(rawCell).toLowerCase();
